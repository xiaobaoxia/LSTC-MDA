...
class PatchMergingTconv(nn.Module):
    def __init__(self, dim_in, dim_out, T=64,kernel_size=7, stride=2, dilation=1):
        super().__init__()
        self.dim_in = dim_in
        ...
        # kernel_size = 3+T
        self.conv2 = nn.Conv2d(dim_in, dim_out, kernel_size=(3+T, 1))

        self.mask_ = torch.ones((3+T, 1))
        self.mask_[3:-3, :] = 0
        self.register_buffer('mask', self.mask_)

        self.mu = nn.Parameter(torch.randn((1,dim_out,T,48)))
    ...
    def forward(self, x):
        ...
        x2 = F.conv2d(x, self.conv2.weight.data * self.mask,self.conv2.bias.data,  padding=(1, 0))#.flatten(2)
...
class SkateFormerBlockDS(nn.Module):
    def __init__(
        ...
        if downscale:
            self.downsample = PatchMergingTconv(in_channels, out_channels, T=t,kernel_size=kernel_size)
        else:
