...
from timm.models.layers import trunc_normal_, Mlp, DropPath, create_act_layer, get_norm_act_layer, create_conv2d
...
class SkateFormerBlock(nn.Module):
    def __init__(self,t, in_channels, num_points=50, kernel_size=7, num_heads=32,
        ...
        # self.tconv = Tconv(in_channels // (2 * 2), in_channels // (2 * 2), kernel_size, num_heads,T=t)
        # Attention layers
        attention = []
    ...
    def forward(self, input):
        ...
        for i in range(len(self.partition_function)):
            C = split_f_attn[i].shape[1] # [32, 36, 64, 48]
...
class PatchMergingTconv(nn.Module):
    def __init__(self, dim_in, dim_out, T=64,kernel_size=7, stride=2, dilation=1,num_points=48):
        super().__init__()
        self.dim_in = dim_in
        self.dim_out = dim_out
        pad = (kernel_size + (kernel_size - 1) * (dilation - 1) - 1) // 2
        # already exist in ori tconv
        self.reduction = nn.Conv2d(dim_in, dim_out, kernel_size=(kernel_size, 1), padding=(pad, 0), stride=(stride, 1),
                                   dilation=(dilation, 1))

        self.bn = nn.BatchNorm2d(dim_out)

        # 803520 params 0.76629638671875 M
        # w=192,96,6 b=192
        # 552960  576
        self.conv2 = nn.Conv2d(dim_in, dim_out, kernel_size=(6, 1))
        # params
        self.zeros_ = torch.zeros(dim_out, dim_in, T-3, 1)

        self.register_buffer('zeros', self.zeros_)

        # 192,48  27648
        self.mu = nn.Parameter(torch.randn((1,dim_out,T,num_points)))
        # 222336 for two linear layer
        self.linear_s = nn.Conv2d(dim_out, dim_out, kernel_size=1)
        self.linear_l = nn.Conv2d(dim_out, dim_out, kernel_size=1)

    def forward(self, x):
        B, C, T, V = x.size()
        x1 = self.reduction(x)#.flatten(2)
        conv2_weight = torch.cat([self.conv2.weight[:, :, :3], self.zeros, self.conv2.weight[:, :, -3:]], dim=2)
        x2 = F.conv2d(x, conv2_weight, self.conv2.bias.data,  padding=(1, 0))#.flatten(2)

        x1_ = self.linear_s(x1)
        x2_ = self.linear_l(x2)
        cos_sim = F.cosine_similarity(x1_, x2_, dim=1).unsqueeze(1)+F.cosine_similarity(self.mu.repeat(B, 1, 1, 1), x2_, dim=1).unsqueeze(1)
        x = x1+cos_sim.softmax(dim=2)*x2
...
class SkateFormerBlockDS(nn.Module):
    def __init__(
            self, t,in_channels, out_channels, num_points=50, kernel_size=7, downscale=False, num_heads=32,
            type_1_size=(1, 1), type_2_size=(1, 1), type_3_size=(1, 1), type_4_size=(1, 1),
            attn_drop=0., drop=0., rel=True, drop_path=0., mlp_ratio=4.,
        ...
        if downscale:
            self.downsample = PatchMergingTconv(in_channels, out_channels, T=t,kernel_size=kernel_size,num_points=num_points)
        else:
...
class SkateFormerStage(nn.Module):
    def __init__(
            self, depth,t, in_channels, out_channels, first_depth=False,
