...
class PatchMergingTconv(nn.Module):
    def __init__(self, dim_in, dim_out, T=64,kernel_size=7, stride=2, dilation=1,num_points=48):
        ...
        # 1080000 params 1.02996826171875 M
        # w=192,96 b=192
        # 313344+331776+184320 + 576 = 830016
        self.conv2 = nn.Conv2d(dim_in, dim_out, kernel_size=(T+1, 1))
        self.mask_ = torch.zeros(dim_out, dim_in, T+1, 1)
        self.mask_[:, :, ::2, :] = 1  # 保留偶数位置，其他置零
    ...
    def forward(self, x):
        B, C, T, V = x.size()
        x1 = self.reduction(x)
        x2 = F.conv2d(x, self.conv2.weight*self.mask, self.conv2.bias.data)#.flatten(2)
