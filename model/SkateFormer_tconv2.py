...
class PatchMergingTconv(nn.Module):
    def __init__(self, dim_in, dim_out, T=64,kernel_size=7, stride=2, dilation=1,num_points=48):
        ...
        self.conv2 = nn.Conv2d(dim_in, dim_out, kernel_size=(4, 1))

        self.zeros_ = torch.zeros(dim_out, dim_in, T-1, 1)
    ...
    def forward(self, x):
        ...
        conv2_weight = torch.cat([self.conv2.weight[:, :, :2], self.zeros, self.conv2.weight[:, :, -2:]], dim=2)
