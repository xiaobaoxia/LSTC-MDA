...
# JMDA data augmentation
from JDA_utils import JDA,JDA_add
...
class LabelSmoothingCrossEntropy(nn.Module):
    ...
    def forward(self, x, target):
        # todo modify this loss to fit JMDA
        confidence = 1. - self.smoothing
        logprobs = F.log_softmax(x, dim=-1)
        # probs = F.softmax(x, dim=-1)
        # logprobs = torch.log(probs).mean(1)
        nll_loss = -logprobs.gather(dim=-1, index=target.unsqueeze(1))
        nll_loss = nll_loss.squeeze(1)
        smooth_loss = -logprobs.mean(dim=-1)
        loss = confidence * nll_loss + self.smoothing * smooth_loss
        return loss.mean()#,probs.mean(1)
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        process = tqdm(loader)
        for batch_idx, (data, index_t, label, index,cam) in enumerate(process):
            ...
            with torch.no_grad():
                ...
                cam = cam.long().cuda(self.output_device)

            timer['dataloader'] += self.split_time()

            label_onehot = torch.nn.functional.one_hot(label, num_classes=self.arg.model_args["num_classes"])
            label_onehot = label_onehot.float()

            index_cam2 = cam == 2
            index_cam3 = ~index_cam2
            data[index_cam2], label_onehot[index_cam2] = JDA_add(data[index_cam2], label_onehot[index_cam2])
            data[index_cam3], label_onehot[index_cam3] = JDA_add(data[index_cam3], label_onehot[index_cam3])
            ...
            for i in range(0, data.shape[0], bs):
                # forward
                output_inner = self.model(data[i:i+bs], index_t[i:i+bs])

                log_probs = F.log_softmax(output_inner, dim=1)
                loss_inner = self.loss_kl(log_probs, label_onehot[i:i+bs])

                # 损失按累积步骤缩放
                loss_inner = loss_inner * output_inner.shape[0] / data.shape[0]
                # backward
                loss_inner.backward()
                ...
                if i == 0:
                    ...
                    output = torch.cat([output, output_inner], dim=0)
                loss = loss + loss_inner.detach()
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, index_t, label, index,cam) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    score_frag.append(probs.data.cpu().numpy())
                    loss_value.append(loss.data.item())

                    _, predict_label = torch.max(probs.data, 1)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                # if epoch + 1 < self.arg.num_epoch * 0.8:
