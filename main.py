...
def get_parser():
    ...
    parser.add_argument('--config', default='./config/train/ntu_cs/SkateFormer_j_test.yaml',
...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.loss_type == 'CE':
            self.loss = nn.CrossEntropyLoss(reduce=True).cuda(output_device)
        else:
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, index_t, label, index) in enumerate(process):
            ...
            for i in range(0, data.shape[0], bs):
                ...
                loss_inner = self.loss(output_inner, label[i:i+bs])
                #
            ...
            process.set_description('Loss: {:.4f}, LR: {:.6f}'.format(loss.data.item(), self.lr))
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            # weights_path = 'pretrained/ntu60_CSub/SkateFormer_j.pt'
