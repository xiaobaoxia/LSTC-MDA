...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path,skes_name):
    ...
    camera_test = camera[train_indices]

    train_samples = skes_name[train_indices]
    test_samples = skes_name[train_indices]

    save_name = 'NTU60_%s.npz' % evaluation
    np.savez(save_name, x_train=train_x, y_train=train_y, x_test=test_x, y_test=test_y,
             camera_train=camera_train,camera_test=camera_test,train_samples=train_samples,test_samples=test_samples)
