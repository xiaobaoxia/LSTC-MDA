...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        if self.split == 'train':
            ...
            self.file_name = npz_data['train_samples']
            self.cam = npz_data['camera_train']

        elif self.split == 'test':
            self.data = npz_data['x_test']
            self.label = np.where(npz_data['y_test'] > 0)[1]
            self.sample_name = ['test_' + str(i) for i in range(len(self.data))]
            self.file_name = npz_data['test_samples']
            self.cam = npz_data['camera_test']
    ...
    def __getitem__(self, index):
        ...
        # copy one subject sample to pad empty
        ...
        if self.split == 'train':
            ...
            # todo may can use JMDA to replace this inter aug
        ...
        return data_numpy, index_t, label, index, cam
